...
public class BattleScript : MonoBehaviourPun
{
    ...
    private void Awake()
    {
        ...
        spinSpeedBar_Image.fillAmount = currentSpinSpeed / startSpinSpeed; // adjust health bar 
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            if (mySpeed > otherPlayerSpeed)
            {
                ...
                // Make sure RPC method call only calls for the player that is supposed to use it
    ...
    public void DoDamage(float _damageAmount)
    {
        if (!isDead)
        {
            // Differentiating the two character types/styles of play
            if (isAttacker)
            {
    ...
    void Die()
    {
        ...
        rb.freezeRotation = false; // spinner will fall over (indicating defeat)
    ...
    IEnumerator ReSpawn()
    {
        ...
        photonView.RPC("ReBorn", RpcTarget.AllBuffered);// rpc method call
    }
